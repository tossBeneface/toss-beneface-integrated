...
class IssueAccessTokenUseCaseTest {
    ...
    fun issueReturnsNewTokenInfo() {
        ...
        given(tokenManager.rotateToken(refreshToken, 42L, Role.USER, mockResponse)).willReturn(jwtTokenDto)
        given(tokenManager.refreshTokenExpirationTime).willReturn("1209600000")

        val response = issueAccessTokenUseCase.issue(refreshToken, mockResponse)
        ...
        verify(tokenManager).rotateToken(refreshToken, 42L, Role.USER, mockResponse)
    }
    ...
    fun issueThrowsExceptionOnInvalidToken() {
        ...
        val thrown = assertThrows(AuthenticationException::class.java) {
            issueAccessTokenUseCase.issue(refreshToken, mockResponse)
        }
